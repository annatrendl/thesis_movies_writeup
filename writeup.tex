...
We then selected the subset of quadruplets where all of the movies had been seen or none of the movies had been seen, to make sure that choice behaviour will not be governed by differences in familiarity with the movies. The result was a bespoke subset of quadruplets for each participant where the target/competitor movies had the same rating and the decoy movies were rated worse. 
 ...
 We then used a sequential method to retrieve the highest quality quadruplets from each participants' subset of quadruplets. This was important, because it often happened that the remaining quadruplets featured the same movie, and we wished to use a movie as a target/competitor just only once for each participant. To obtain this, we first chose the quadruplet with the smallest combined target-decoy semantic distance, then eliminated the quadruplets with the same target or competitor movies, and repeated the first and second step until all the potential quadruplets have been eliminated, and a subset of these have been chosen. We then only invited those participants back for whom we could create at least three high quality quadruplets.
